...
export default function F1Screen() {
  ...
  useEffect(() => {
    ...
    const off = wsOnMessage((msg: any) => {
      // 1) 위치/경로 갱신
      if (String(msg.floor || '').toUpperCase() === '1F') {
      ...
      // 2) 🔥 화재 알림
      if (msg?.kind === 'fire_alert' && String(msg.floor || '').toUpperCase() === '1F') {
        const now = Date.now();
        ...
        if (Platform.OS !== 'web') {
          Alert.alert('화재 감지', `1F층에 화재가 발생했습니다.\n신속히 대피하세요!`);
        }
      }

      // 3-a) 개별 이벤트 형식: { kind:'hazard', floor, node:[x,y], active:true/false }
      if (msg.kind === 'hazard' && String(msg.floor || '').toUpperCase() === '1F'
      ...
      // 3-b) 상태 스냅샷 형식: { kind:'hazard_state', floor:'1F', hazard_nodes:[[x,y], ...] }
      if (msg.kind === 'hazard_state'
          && String(msg.floor || '').toUpperCase() === '1F'
      ...
      {fireAlert && (
        <View style={styles.banner}>
          <Text style={styles.bannerTxt}>
            🔥 화재 감지(1F) conf={fireAlert.confidence.toFixed(2)}
          </Text>
