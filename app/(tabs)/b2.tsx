  ...
  useEffect(() => {
    ...
    const off = wsOnMessage((msg: any) => {
      ...
      if (msg?.kind === 'fire_alert' && String(msg.floor || '').toUpperCase() === 'B2') {
        ...
        if (Platform.OS !== 'web') {
          Alert.alert('화재 감지', `B2층에 화재가 발생했습니다.\n신속히 대피하세요!`);
        }
      }

      // 3-a) 개별 이벤트 형식: { kind:'hazard', floor, node:[x,y], active:true/false }
      if (msg.kind === 'hazard' && String(msg.floor || '').toUpperCase() === 'B2'
      ...
      // 3-b) 상태 스냅샷 형식: { kind:'hazard_state', floor:'B2', hazard_nodes:[[x,y], ...] }
      if (msg.kind === 'hazard_state'
          && String(msg.floor || '').toUpperCase() === 'B2'
